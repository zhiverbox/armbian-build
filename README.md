...
zHIVErbox armbian build tools customizations</h3>
...
repository](https://github.com/zhiverbox/armbian-userpatches).
...
- [zHIVErbox changes](#zhiverbox-changes)
...
The following modifications can't be done via the userpatches and have to be made in the build tools:
...
5. minor changes
5.1. avoid username leaks in `armbian-check-first-login.sh`
5.2. use netcat with Tor socks `proxy in armbian-config.sh`
5.3. use torsocks to pull zHIVErbox git updates in `compile.sh`
